...
class MTProtoProxy:
    ...
    TELEGRAM_SERVERS = {
        ...
        2: [('149.154.167.51', 443), ('149.154.167.50', 443)],
        3: [('149.154.175.100', 443)],
        4: [('149.154.167.91', 443), ('149.154.167.92', 443)],
        5: [('149.154.171.5', 443)],
    }
    ...
    async def handle_client(self, client_reader, client_writer):
        ...
        try:
            ...
            # Выбираем DC (по умолчанию DC2)
            dc_id = 2
            telegram_host, telegram_port = self.TELEGRAM_SERVERS[dc_id][0]
            ...
            telegram_writer.write(handshake)
            await telegram_writer.drain()
            
            # Проксируем данные в обе стороны
            await asyncio.gather(
                self.pipe_data(client_reader, telegram_writer, "client->telegram"),
                self.pipe_data(telegram_reader, client_writer, "telegram->client"),
                return_exceptions=True
            )
            
        except asyncio.TimeoutError:
            logger.warning("Таймаут при подключении")
